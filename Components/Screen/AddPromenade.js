...
class AddPromenade extends Component {
	...
	render() {
		...
		const geoloca = {
			description: 'Ma location',
			geometry: { location: { lat: this.state.currentlatitude, lng: this.state.currentlongitude } },
		};
		if (!this.props.user.token) {
			...
			return (
				<Container>
					<Content style={styles.container}>
						<View style={{ alignItems: 'center', justifyContent: 'center' }}>
							<Thumbnail square large source={{ uri: this.props.user.avatar }}></Thumbnail>
							<Text>
								{this.props.user.name} & {this.props.user.dog1}
							</Text>
						</View>
              ...
              Date: {this.state.chosenDate.toString().substr(4, 12)}
						...
						<Text style={styles.title}>Durée approximative:</Text>
...
export default connect(mapStateToProps, null)(AddPromenade);
