...
class Home extends React.Component {
	...
	componentDidMount() {
		AsyncStorage.getItem('userComeback', (err, data) => {
			if (data) {
	...
	static navigationOptions = ({ navigation }) => ({
		...
		headerTitle: <Image source={require('../../assets/Images/dogo.png')} style={{ width: 84, height: 33 }} />,
	});
			...
			<ImageBackground style={{ height: '100%' }} source={require('../../assets/Images/home5.jpg')}>
				<View style={{ flex: 1, alignItems: 'center' }}>
					{this.state.fontLoaded ? (
						...
						<Text style={{ fontSize: 25, textAlign: 'center' }}>Des compagnons de promenade</Text>
					)}
					...
					{this.state.fontLoaded ? (
						...
						<Text style={{ fontSize: 25, textAlign: 'center' }}>autour de vous</Text>
					)}
				</View>
				...
				<View>
					<Button
						info
						style={{ marginHorizontal: 85, marginBottom: 10, position: 'center' }}
					...
					<Button
						info
						style={{ marginHorizontal: 80, marginBottom: 20, position: 'center' }}
